...
- 现有 pdf_summarizer 的 `Document` / `Page` 不动，只留一条桥接路径（已落地：`extractor/paddle_vl.py`，§7 第 3 步）；

非目标（本次刻意排除）：

- 不做插件注册表、多后端编排、统一异步框架；
- 不兼容 PaddleOCR 2.x 旧 API——旧 `paddle_ocr.py` 已整体删除、由新桥接替换（2026-09-21），不是改造；
...
class PaddleOCRVLConfig:               # 只暴露常用开关，其余透传 extra_params
    ...
    model_dir: str | Path | None = None    # None=自动优先仓库快照（data/ocr_backend/ 模型库），显式路径则覆盖
...
src/ocr_backend/
  ...
  models.py              # 模型库约定：MODELS 注册表、model_dir(name)、is_ready(dir)
  cli.py                 # `ocr-backend download <name>`：按注册表一键下载就位
  backends/
...
tests/test_ocr_backend/
  ...
  test_cli.py            # 注册表 / 下载路径 / 就位判据，mock 掉网络与模型
...
3. pdf_summarizer 迁移试验：删旧 `paddle_ocr.py`，extractor 接新层，用一张扫描件跑端到端 —— **已完成**（桥接 `src/pdf_summarizer/extractor/paddle_vl.py`；`create_extractor` 支持 `paddleocr-vl` 与 `auto` 回退链。端到端素材 `data/ocr_backend/verify/materials/scanned_verify_doc.pdf`（5 页纯图、文本层 0 字符）：PyMuPDF 单独提取报 "No extractable text found"，OCR 后端 5 页 1311 字符 / 冷跑 64.5s，`auto` 链回退结果逐字节一致；复跑脚本 `data/ocr_backend/verify/e2e_extractor_migration.py`。`pdf2image` 死依赖同步删除）；
