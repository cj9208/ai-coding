...
日期：2026-09-22。状态：设计稿，`src/photo_desk/` 未开工。读法：§1–§2 是需求与已定裁决（背景）；§3–§4 是实现契约（开工后以本节为准）；§5 是里程碑与验收标准；§6–§7 是风险与依赖。
...
| 里程碑 | 内容 | 验收 |
|---|---|---|
| **M0 读懂** | config / scan / exif / library / 时间线页 | 对一个含数百张真实照片的测试目录：增量扫描二次运行只 stat 不改的行；时间线按 EXIF 拍摄日（非文件 mtime）分组正确；HEIC/JPG 混排可读 |
| **M1 筛选**（诉求①） | quality / groups / triage + restore CLI | 构造 golden 连拍组（含人为模糊样张）：组识别全对、垫底样张被隔离且账本可放回；`restore --group` 与 sha256 不符拒绝路径均有测试；真实目录演练一次，隔离清单先出、人工确认后执行（首次执行保留 `--dry-run` 默认开） |
...
2. 缩略图尺寸策略（列表页/详情页两档？HEIC 内嵌 full-size preview 可否直接白嫖）——M0 实施时定；
