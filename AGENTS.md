...
| `src/rag/` | knowledge pipeline over OCR output: `rag build` (ingest bundles → validate gate → chunk → FTS5 index → publish snapshot), `rag query` (lexical retrieval → RRF → evidence pack → thin grounded answer with 5 outcomes + citation validation), `rag eval` (golden set). M1 is deliberately **vector-free** — embeddings attach later to content-addressed chunk ids via a new `CandidatePath` plug-in and a backfill job, nothing else changes (`embeddings` table slot is empty); representations live per-snapshot in `snapshots.representations`. Publishes a markdown projection of every doc to `data/rag/corpus/` as a grep-able fallback. Design doc `docs/rag-subsystem-design.md`; sample golden file `tests/golden/rag_sample.jsonl`. Run tracing: `--trace` on build/query/eval records OTel-shaped spans (model copied, no dependency) as one JSONL per run under `data/rag/traces/`, read back with `rag traces [FILE]`; instrumentation is wrapper shells at the composition roots only — stage modules never see the tracer (`src/rag/tracing.py`) | yes, query-time + opt-in `build --enrich`, via `llm_client` |
