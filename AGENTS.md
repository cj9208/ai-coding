...
- Change dependencies with `uv add` / `uv remove`, then commit the refreshed
  ...
  **Gotcha (2026-09-23):** `uv add` runs an implicit `uv sync` *without extras*,
  so adding any dependency silently uninstalls `paddleocr` / `paddlepaddle-gpu`
  from this venv — restore with
  `uv sync --locked --extra ocr --extra paddle-gpu` after a dependency edit.
...
| `src/notify/` | shared notification layer (contract: `docs/notify-design.md`, rulings D-1..D-6 — do not re-derive): task code calls one facade `notify.emit(project, kind, **payload)` which appends a structured event to the SQLite ledger at `data/notify/events.db` and **never raises**; delivery happens only in short-lived `notify dispatch` (scheduler-launched, never a daemon), channels are pluggable adapters sharing one shape (stdout + **telegram** since M1, ping/email in M2; telegram is gated on `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` in `.env`, and `TELEGRAM_PROXY` is **optional — an empty value means direct**, which is what this machine needs, verified 2026-09-23: it reaches `api.telegram.org` without one); "who watches the watcher" is closed at four stacked failure domains L1-L4, L4 being the human inversion "no daily digest = something is wrong" — the acknowledged limit, no VPS for it; **M0 shipped 2026-09-22**, **M1 shipped 2026-09-23** (`policy.py` priority/throttle/escalation as a pure function pinned by 8 golden cases in `tests/golden/notify_policy.jsonl`; `rules.py` + tracked `config/notify/expectations.yaml` where silence-is-a-gap is declared as data — its one rule ships `enabled: false` until `quant record` is scheduled; `deliveries.attempts` drives the 3-strikes escalation, and the `notify:delivery.failed` self-alert is throttled so a dead channel costs one message an hour, not one per round; `notify check` dry-runs config+rules and `--send` probes the wire) — 105 tests, the four §5 acceptance items all run against a real phone, evidence table in §5; CLI `notify emit / status / dispatch / check / schedule`; **`schedule.py` wraps Windows Task Scheduler so the §4.9 registration is one command instead of a retyped PowerShell block** (paths derived from `REPO_ROOT`, stdlib-only, refuses non-Windows hosts with the cron/systemd recipe rather than pretending; `install` / `status` / `run-now` / `remove`); **the task is registered on this machine and its first unattended round ran with nobody typing `dispatch` (2026-09-23, evidence §4.9)**, owed: the closed-lid sleep-wake catch-up is still paper logic, not observed (TODO.md) | no |
...
| `src/photo_desk/` | 家庭照片管理（设计契约 `docs/photo-desk-design.md`，裁决 D-1..D-5 在 §2 勿重新推导）——NAS 目录只读 + 可逆筛查 + 无账号时间线；**M0 shipped 2026-09-22**（config 双根地址 PHOTO_ROOT/PHOTO_DESK_DATA_DIR、DB 只存相对路径供迁 NAS、mtime+size 先筛 sha256 后算的增量 scan、Pillow+pillow-heif EXIF/GPS/亚秒提取、512/1600 两档 WebP 派生缓存、FastAPI+Jinja 时间线按拍摄日分组），真机演练在 `data/photo_desk_demo/`（gitignored），HEIC 真读与 NAS 挂载验收欠账见 TODO.md；**M1 shipped 2026-09-23**（quality.py 拉普拉斯方差清晰度分——numpy 手搓核、不引 opencv；groups.py 三级连拍归组，规则由 `tests/golden/photo_groups.jsonl` 锁死；triage.py 全系统唯一写通道：dry-run 默认、组内相对排名 <0.5×best 才隔离、移入 `blurred/uuid名`+move_ledger 落行、restore 先验 sha256（篡改/占位拒放、孤儿重建父目录），CLI `photos triage [--apply]` / `photos restore --photo|--group`，时间线灰显 quarantined、放回按钮归 M2）；M2 tag+FTS+编辑模式未开工 | no |
