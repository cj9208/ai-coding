...
Check `specs/openspec/changes/{change-id}/tasks.md`:
...
For each file in `specs/openspec/changes/{change-id}/specs/`:

#### 3.1 Check if target spec exists

- If `specs/openspec/specs/{component}.md` exists → Merge changes
...
mkdir -p specs/openspec/archive/{change-id}

# Move all change files
mv specs/openspec/changes/{change-id}/* specs/openspec/archive/{change-id}/
rmdir specs/openspec/changes/{change-id}
```

### Step 5: Update Archive Metadata

Add completion info to `specs/openspec/archive/{change-id}/proposal.md`:
...
### Specs Updated
- `specs/openspec/specs/{component}.md`
```

### Step 6: Commit the Archive

```bash
git add specs/openspec/
...
### Specs Updated
- `specs/openspec/specs/{component}.md` - {summary of changes}

### Archive Location
- `specs/openspec/archive/{change-id}/`
